...
### Fixed
- GitHub Actions release workflow: fixed `tauri-action` using `pnpm` instead of `bun` for builds
  - Added `tauriScript: bun run tauri` to explicitly specify the package manager
  - Committed `bun.lock` to the repository so auto-detection works correctly
